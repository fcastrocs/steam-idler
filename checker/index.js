...
class Checker {
	...
	PrepareToWriteToFile(account, client) {
		...
		if (account.hit) {
			...
			if (account.emailVerified || account.communityBannedLocked || account.limited) {
	...
	WriteAccount(account) {
		...
		if(!account.hit && account.sameDomains){
			data = `${account.user}:${account.pass}\n`
			file = `${path}/to-check-later.txt`;
		...
		// good hit vacced
		if(account.hit && account.goodHit && account.vac){
			data += `vacced\n`;
		}

		if(account.hit && !account.goodHit){
			...
			if(account.communityBannedLocked){
				data += `community banned or locked\n`;
			}
		}

		if(account.hit && account.hasGames){
			...
			account.gamesObj.games.forEach((game) =>{
				data += ` + ${game.name} [${game.billingtype}]\n`;
			})
		}else{ // Not a hit
		...
		if(account.hit){
			if(account.goodHit){
				...
				if(account.emailVerified && !account.limited && !account.communityBannedLocked){
					file = `${path}/hits/email-verified.txt`;
				}else if(account.limited && !account.emailVerified && !account.communityBannedLocked){
					file = `${path}/hits/${account.digits}Dig-limited.txt`;
				}else{
