...
export default class Connection extends EventEmitter {
   ...
   */
  public async connect(options: SocksClientOptions): Promise<void> {
    ...
    try {
      ...
      this.socket.setTimeout(Number(process.env.SOCKET_TIMEOUT));
   ...
   */
  public send(message: Buffer | AnyObject, EMsg?: number): void {
  ...
  private async decodePacket(packet: Buffer) {
    ...
    if (EMsg === Language.EMsg.Multi) {
      ...
      if (EMsg === Language.EMsg.ClientVACBanStatus) {
        ...
        this.emit('CMsgClientVACBanStatus', { vacbanned: bans === 0 ? false : true });
      }
