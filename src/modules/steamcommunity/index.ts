...
import { IProxy, IProxyDoc } from '../../models/proxy';
 ...
 */
export async function login(steamid: string, webNonce: string, proxy: IProxyDoc | IProxy): Promise<Cookie> {
 ...
 */
export async function getFarmingData(steamid: string, cookie: Cookie, proxy: IProxyDoc | IProxy): Promise<FarmData[]> {
 ...
 * Get cards inventory
 */
export async function getCardsInventory(steamid: string, cookie: Cookie, proxy: IProxyDoc | IProxy): Promise<Item[]> {
